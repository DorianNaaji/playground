...
public class Elves {
  ...
  /**
   * Put elves at work to compute the square feet of wrapping paper
   * they need.
  ...
  Answer computeAnswer(List<String> dimensions) throws IOException, ElfComputingException {
    ...
    Long requiredPaper = presents.stream().mapToLong(PresentBox::getRequiredPaperInSquareFeet).sum();
    Long requiredRibbonLength = presents.stream().mapToLong(PresentBox::getRequiredRibbonLength).sum();
